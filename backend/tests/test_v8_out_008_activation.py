...
from app.modules.documents.official_notice_catalog import (
    seed_official_letter_out_form_008_catalog as seed_doc_templates,
)
...
def test_seed_accumulates_form_008_internal_only_after_forms_001_through_007(
            ...
            transaction.execute(
                ...
                .where(
                    DocTemplate.code.in_(
                        tuple(f"OFFICIAL_LETTER_OUT_{index:03d}" for index in range(1, 9))
                    )
                )
                .order_by(DocTemplate.code.asc())
            )
