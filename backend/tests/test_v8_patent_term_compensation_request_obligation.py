...
LINEAGE_KEY = "patent-term-compensation-request-submission"
WRONG_LINEAGE_KEY = "ic-layout-remuneration-adjudication-request-submission"
REVIEWED_AT = datetime(2026, 7, 25, 10)
REQUESTED_AT = datetime(2026, 7, 25, 10, 30)
...
def _boundary():
    command_type = getattr(
        fee_linking_service,
        "RecognizePatentTermCompensationRequestObligationCommand",
        None,
    )
    recognize = getattr(
        fee_linking_service,
        "recognize_patent_term_compensation_request_obligation",
        None,
    )
...
def _seed_case(transaction: Session) -> None:
    transaction.add(
        Case(
            id=CASE_ID,
            case_no="PATENT-TERM-COMPENSATION-REQUEST",
            case_type="NORMAL",
            patent_category="INV",
    ...
    *,
    ...
    transaction.add(
        DocAttachment(
            ...
            file_name=f"patent-term-compensation-request-{ordinal}.xml",
            file_path=f"/evidence/patent-term-compensation-request-{ordinal}.xml",
    ...
    *,
    ...
    result = evidence_workflow.finalize_external_submission(
        evidence_workflow.FinalizeExternalSubmissionCommand(
            ...
            actor_id=ACTOR_ID,
            submitted_at=requested_at or REQUESTED_AT + timedelta(days=ordinal),
            idempotency_key=f"patent-term-compensation-request-{ordinal}",
        ),
        transaction,
    )
    transaction.commit()
...
def _expect_conflict(callable_: object) -> None:
    ...
    assert captured.value.code == "PATENT_TERM_COMPENSATION_REQUEST_SOURCE_CONFLICT"


def test_each_reviewed_final_request_forms_and_reuses_only_its_own_fee(
    ...
    try:
        ...
        assert {
            ...
            (
                ...
                Decimal("200.00"),
                None,
                (REQUESTED_AT + timedelta(days=1)).date(),
                "MATCHED",
            ),
            (
                "CN_PATENT_TERM_COMPENSATION_REQUEST_FEE",
                ...
                Decimal("200.00"),
                None,
                (REQUESTED_AT + timedelta(days=2)).date(),
                "MATCHED",
            ),
        }
        assert _counts(transaction) == (2, 2, 0, 0, 0, 2)
        assert [
            (activity.activity_type, activity.confirmation_status)
            for activity in transaction.scalars(
                select(CaseActivityEvent).order_by(CaseActivityEvent.sequence)
            )
        ] == [
            ("DOCUMENT_EVIDENCE_EXTERNAL_SUBMISSION_FINALIZED", "CONFIRMED"),
            ("FEE_OBLIGATION_RECOGNIZED", "NEEDS_REVIEW"),
            ("DOCUMENT_EVIDENCE_EXTERNAL_SUBMISSION_FINALIZED", "CONFIRMED"),
            ("FEE_OBLIGATION_RECOGNIZED", "NEEDS_REVIEW"),
        ]
    finally:
...
@pytest.mark.parametrize(
    ...
    (
        ...
        "TERM_COMPENSATION_REQUEST_INTENT_RECORDED",
    ),
)
...
def test_request_date_selects_the_fee_rule_and_pre_effective_date_writes_nothing(
    session_factory: sessionmaker[Session],
) -> None:
    transaction = session_factory()
    try:
        _seed_case(transaction)
        evidence_id = _seed_evidence(transaction, ordinal=1)
        activity_id = _finalize(
            transaction,
            evidence_id=evidence_id,
            ordinal=1,
            requested_at=datetime(2024, 8, 5, 23, 59),
        )
        _, recognize = _boundary()

        with pytest.raises(BusinessError) as captured:
            recognize(_command(activity_id, evidence_id), transaction)

        assert captured.value.status_code == 400
        assert captured.value.code == "PATENT_TERM_COMPENSATION_REQUEST_FEE_UNAVAILABLE"
