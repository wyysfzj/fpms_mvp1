...
def _rows(transaction: object) -> list[DocTemplate]:
    return (
        transaction.execute(
            select(DocTemplate)
...
def test_seed_classifies_only_form_019_as_internal_reference_material(
    ...
    with session_factory() as transaction:
        ...
        for index, row in enumerate(rows[:19], start=1):
        ...
        row = rows[18]
        assert (row.code, row.name) == ("OFFICIAL_LETTER_OUT_019", "二通意见陈述")
        assert json.loads(row.input_fields or "{}") == {
            ...
            "legacy_form_scope": "form-019",
            "official_letter_name": "二通意见陈述",
