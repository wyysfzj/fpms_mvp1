...
LINEAGE_KEY = "ic-layout-reexamination-request-submission"
REGISTRATION_LINEAGE_KEY = "ic-layout-registration-submission"
...
def _boundary():
    command_type = getattr(
        fee_linking_service,
        "RecognizeIcLayoutReexaminationRequestObligationCommand",
        None,
    )
    recognize = getattr(
        fee_linking_service,
        "recognize_ic_layout_reexamination_request_obligation",
        None,
    )
    if command_type is None or recognize is None:
        pytest.skip("IC-layout reexamination-request recognition seam is not implemented")
...
def _seed_case_and_rate(transaction: Session) -> None:
    transaction.add(
        Case(
            id=CASE_ID,
            case_no="IC-LAYOUT-REEXAM-REQUESTED",
            case_type="NORMAL",
            patent_category="INV",
...
def _expect_conflict(callable_: object) -> BusinessError:
    ...
    assert captured.value.code == "IC_LAYOUT_REEXAM_REQUEST_SOURCE_CONFLICT"
...
def test_explicit_recognition_writes_exact_obligation_and_only_owned_fee_activity(
    ...
    try:
        ...
        active_rate = Mock(wraps=fee_linking_service.get_layout_reexamination_fee)
        ...
        assert result.idempotency_key == "ic-layout-reexamination-request"
        ...
        assert active_rate.call_count == 1
        assert len(active_rate.call_args.args) == 1
...
def test_registration_lineage_is_source_conflict_without_obligation_line_or_fee_activity(
    session_factory: sessionmaker[Session],
) -> None:
    _, recognize = _boundary()
    with session_factory() as transaction:
        ...
        evidence.lineage_key = REGISTRATION_LINEAGE_KEY
        evidence.current_identity_key = f"{CASE_ID}|{REGISTRATION_LINEAGE_KEY}"
        transaction.commit()
        source_activity_id = _finalize(transaction)
        before = _counts(transaction)
        assert before == (1, 1, 0, 0, 0, 0, 0)

        error = _expect_conflict(
            lambda: recognize(
                _command(source_activity_id=source_activity_id),
                transaction,
            )
        ...
        assert error.code == "IC_LAYOUT_REEXAM_REQUEST_SOURCE_CONFLICT"
        assert _counts(transaction) == before
        assert (
            transaction.scalar(
                select(func.count())
...
@pytest.mark.parametrize(
    ("target", "field", "value"),
    (
        ("activity", "activity_type", "APPLICATION_REJECTION_CONFIRMED"),
        ("activity", "activity_type", "REEXAMINATION_STARTED"),
...
def test_rejects_wrong_or_dirty_transaction_before_source_reads(
    ...
    assert wrong.value.code == "IC_LAYOUT_REEXAM_REQUEST_INVALID"
...
def test_distinct_second_finalized_source_is_409_with_zero_writes(
    ...
    with session_factory() as transaction:
        ...
        _seed_evidence(
            ...
            evidence_id=_id(12),
            lineage_key="ic-layout-reexamination-request-submission-second",
        )
        transaction.commit()
