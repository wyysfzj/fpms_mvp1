...
class LayoutReexaminationFeeRuleTests(unittest.TestCase):
    ...
    def test_returns_exact_source_bound_fee_on_and_after_effective_date(self) -> None:
        ...
        for effective_date in (date(2017, 7, 1), date(2035, 12, 31)):
            with self.subTest(effective_date=effective_date):
                ...
                self.assertEqual(result.fee_code, "IC_LAYOUT_REEXAM_REQUEST_FEE")
                ...
                self.assertEqual(
                    caught.exception.details,
                    {
                        "field": (
                            "command"
                            if type(command)
                            is not official_rate_book.GetLayoutReexaminationFeeCommand
                            else "effective_date"
                        )
                    },
                )
