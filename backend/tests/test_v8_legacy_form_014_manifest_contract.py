...
MANIFEST = ROOT / "tasks" / "batches" / "FPMS-POSTDEMO-V8-LEGACY-FORM-014-GATE-20260712-01.md"

ACTIVATION_TASK = "tasks/postdemo/v8/FPMS-V8-LEGACY-FORM-014-MANIFEST-ACTIVATION-20260712-01.md"
OUT_014_TASK = "tasks/postdemo/v8/FPMS-V8-OUT-014-FEE-REDUCTION-REQUEST-20260712-01.md"
...
def _manifest_text() -> str:
    assert MANIFEST.is_file(), f"missing form-014 lane manifest: {MANIFEST}"
    return MANIFEST.read_text(encoding="utf-8")


def test_manifest_contains_only_activation_then_out_014() -> None:
    text = _manifest_text()
    declared_task_files = re.findall(r"^- Task file: `([^`]+)`$", text, re.MULTILINE)

    assert re.findall(r"^Task count: (\d+)$", text, re.MULTILINE) == ["2"]
    assert declared_task_files == [ACTIVATION_TASK, OUT_014_TASK]


def test_manifest_binds_internal_only_authority_and_prerequisites() -> None:
    text = _manifest_text()

    assert "DG-LEGACY-FORM-CLASS:form-014" in text
