...
MANIFEST = ROOT / "tasks" / "batches" / "FPMS-POSTDEMO-V8-LEGACY-FORM-009-GATE-20260712-01.md"

ACTIVATION_TASK = "tasks/postdemo/v8/FPMS-V8-LEGACY-FORM-009-MANIFEST-ACTIVATION-20260712-01.md"
OUT_009_TASK = "tasks/postdemo/v8/FPMS-V8-OUT-009-VOLUNTARY-RECTIFICATION-20260712-01.md"
...
def _manifest_text() -> str:
    assert MANIFEST.is_file(), f"missing form-009 lane manifest: {MANIFEST}"
    return MANIFEST.read_text(encoding="utf-8")


def test_manifest_contains_only_activation_then_out_009() -> None:
    text = _manifest_text()
    declared_task_files = re.findall(r"^- Task file: `([^`]+)`$", text, re.MULTILINE)

    assert re.findall(r"^Task count: (\d+)$", text, re.MULTILINE) == ["2"]
    assert declared_task_files == [ACTIVATION_TASK, OUT_009_TASK]


def test_manifest_binds_internal_only_authority_and_prerequisites() -> None:
    text = _manifest_text()

    assert "DG-LEGACY-FORM-CLASS:form-009" in text
...
def test_manifest_preserves_reference_only_and_distinct_scope_boundaries() -> None:
    text = _manifest_text()
    ...
    assert "Form-017 membership in this lane: none" in text
