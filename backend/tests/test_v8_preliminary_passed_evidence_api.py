...
PATH = "/api/v1/documents/{document_id}/lifecycle/preliminary-pass"
ROUTER_PATH = "/documents/{document_id}/lifecycle/preliminary-pass"
CASE_ID = "case-preliminary-pass-api"
DOCUMENT_ID = "document-preliminary-pass"
ATTACHMENT_ID = "attachment-preliminary-pass"
EVIDENCE_VERSION_ID = "evidence-preliminary-pass"
OTHER_CASE_ID = "case-preliminary-pass-other"
...
EFFECTIVE_AT = datetime(2026, 7, 21, 10, 30)
OCCURRED_AT = datetime(2026, 7, 21, 10, 15)
REVIEWED_AT = datetime(2026, 7, 21, 9, 45)
...
def _request_data(
    *,
    ...
    effective_at: str = "2026-07-21T10:30:00",
    occurred_at: str | None = None,
    idempotency_key: str = "preliminary-pass-1",
) -> dict[str, object]:
...
def _case(
    *,
    case_id: str = CASE_ID,
    case_no: str = "V8-PRELIMINARY-PASS-API",
) -> Case:
    return Case(
        id=case_id,
        case_no=case_no,
...
def _seed_valid_evidence(session_factory: sessionmaker) -> None:
    with session_factory() as transaction:
        ...
        _add_document_evidence(
            ...
            attachment_id=ATTACHMENT_ID,
            evidence_version_id=EVIDENCE_VERSION_ID,
            lineage_key="preliminary-pass-notice",
        )
        transaction.commit()
        ...
        if relationship == "same_document":
            transaction.add(
                DocAttachment(
                    ...
                    document_id=DOCUMENT_ID,
                    file_name="preliminary-pass-alt.pdf",
                    file_path="/evidence/preliminary-pass-alt.pdf",
                    mime_type="application/pdf",
                    content_hash=CONTENT_HASH,
                )
            )
            ...
            transaction.add(
                DocumentEvidenceVersion(
                    ...
                    lineage_key="preliminary-pass-notice-alt",
                    role="OFFICIAL_FINAL_PDF",
                    version_number=1,
                    state="FINAL",
                    creator_id=CREATOR_ID,
                    ...
                    reviewed_at=REVIEWED_AT,
                    content_hash=CONTENT_HASH,
                    current_identity_key=f"{CASE_ID}|preliminary-pass-notice-alt",
                )
            )
            ...
            _add_document_evidence(
                ...
                lineage_key="preliminary-pass-other-document",
            )
        elif relationship == "other_case":
            transaction.add(
                _case(
                    case_id=OTHER_CASE_ID,
                    case_no="V8-PRELIMINARY-PASS-OTHER",
                )
            )
            transaction.flush()
            _add_document_evidence(
                ...
                attachment_id=OTHER_ATTACHMENT_ID,
                evidence_version_id=OTHER_EVIDENCE_VERSION_ID,
                lineage_key="preliminary-pass-other-case",
            )
        else:
...
def test_route_request_and_response_freeze_the_exact_public_contract() -> None:
    route = _route()
    ...
    assert payload_type.model_validate(_request_data()).model_dump() == {
        ...
        "effective_at": EFFECTIVE_AT,
        "occurred_at": None,
        "idempotency_key": "preliminary-pass-1",
    }
    assert get_type_hints(route.response_model) == {
        "case_id": str,
        "document_id": str,
        "evidence_version_id": str,
        "activity_id": str,
...
@pytest.mark.parametrize(
    ...
    (
        ...
        {**_request_data(), "event_type": "PRELIMINARY_EXAMINATION_PASSED"},
        ...
        {**_request_data(), "effective_at": "2026-07-21T10:30:00+08:00"},
        {**_request_data(), "occurred_at": "2026-07-21T10:15:00+08:00"},
    ),
)
def test_missing_extra_client_owned_wrong_type_and_aware_inputs_are_exact_422(
...
def test_confirmed_pass_evidence_invokes_only_the_exact_lifecycle_event_and_commits_once(
    session_factory: sessionmaker,
    ...
    def apply(
        ...
        return LifecycleTransitionResult(
            case_id=CASE_ID,
            activity_id="activity-preliminary-pass",
            sequence=1,
            ...
            legacy_case_status="PRELIM_PASS",
    ...
    with session_factory() as transaction:
        ...
        with _client_for_transaction(recording) as client:
            response = client.post(
                ...
                json=_request_data(occurred_at="2026-07-21T10:15:00"),
            )
    ...
    assert response.json() == {
        ...
        "activity_sequence": 1,
        "lifecycle_revision": 1,
        "effective_at": "2026-07-21T10:30:00",
        "occurred_at": "2026-07-21T10:15:00",
        "idempotency_key": "preliminary-pass-1",
        "reused": False,
    }
    assert captured == [
...
@pytest.mark.parametrize("occurred_at", (None, "2026-07-21T10:15:00"))
def test_fresh_and_replay_persist_one_pass_fact_without_changing_the_four_axis_projection(
    session_factory: sessionmaker,
    auth_headers: dict[str, str],
    ...
    with session_factory() as transaction:
        ...
        assert [
            ...
            (
                "PRELIMINARY_EXAMINATION_PASS_NOTICE",
                "DocumentEvidenceVersion",
                EVIDENCE_VERSION_ID,
                CONTENT_HASH,
                REVIEWED_AT,
...
def test_wrong_document_or_case_relationship_is_exact_400_without_partial_status(
    ...
    assert response.json()["error"]["code"] == "PRELIMINARY_PASS_EVIDENCE_RELATION_MISMATCH"
...
def test_stored_evidence_conflicts_are_exact_409_without_partial_status(
    ...
    assert response.json()["error"]["code"] == "PRELIMINARY_PASS_EVIDENCE_CONFLICT"
    assert response.json()["error"]["details"] == {"field": "evidence_version_id"}
    _assert_case_unchanged(session_factory)
...
def test_same_key_with_changed_command_or_evidence_is_idempotency_conflict(
    ...
    changed_payload = (
        _request_data(effective_at="2026-07-21T10:31:00")
...
def test_authentication_and_doc_edit_permission_preserve_401_and_403(
    ...
    user_id = "preliminary-pass-no-permission"
    with session_factory() as transaction:
        transaction.add(
            T_User(
                id=user_id,
                username="preliminary-pass-no-perm",
                display_name="无权限用户",
