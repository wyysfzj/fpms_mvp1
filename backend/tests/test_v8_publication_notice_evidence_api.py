...
PATH = "/api/v1/documents/{document_id}/lifecycle/publication-notice"
ROUTER_PATH = "/documents/{document_id}/lifecycle/publication-notice"
CASE_ID = "case-publication-api"
DOCUMENT_ID = "document-publication-api"
ATTACHMENT_ID = "attachment-publication-api"
EVIDENCE_VERSION_ID = "evidence-publication-api"
ALT_ATTACHMENT_ID = "attachment-publication-alt"
ALT_EVIDENCE_VERSION_ID = "evidence-publication-alt"
OTHER_CASE_ID = "case-publication-other"
OTHER_DOCUMENT_ID = "document-publication-other"
OTHER_ATTACHMENT_ID = "attachment-publication-other"
OTHER_EVIDENCE_VERSION_ID = "evidence-publication-other"
ACTOR_USERNAME = "admin"
CREATOR_ID = "publication-creator"
...
def _request_data(
    *,
    ...
    idempotency_key: str = "publication-notice-1",
) -> dict[str, object]:
...
def _case(
    *,
    case_id: str = CASE_ID,
    case_no: str = "V8-PUBLICATION-API",
) -> Case:
    return Case(
        id=case_id,
        case_no=case_no,
...
def _seed_valid_evidence(session_factory: sessionmaker) -> None:
    with session_factory() as transaction:
        ...
        _add_document_evidence(
            ...
            attachment_id=ATTACHMENT_ID,
            evidence_version_id=EVIDENCE_VERSION_ID,
            lineage_key="publication-notice",
        )
        transaction.commit()
        ...
        if relationship == "same_document":
            transaction.add(
                DocAttachment(
                    ...
                    document_id=DOCUMENT_ID,
                    file_name="publication-alt.pdf",
                    file_path="/evidence/publication-alt.pdf",
                    mime_type="application/pdf",
                    content_hash=CONTENT_HASH,
                )
            )
            ...
            transaction.add(
                DocumentEvidenceVersion(
                    ...
                    lineage_key="publication-notice-alt",
                    role="OFFICIAL_FINAL_PDF",
                    version_number=1,
                    state="FINAL",
                    creator_id=CREATOR_ID,
                    ...
                    reviewed_at=REVIEWED_AT,
                    content_hash=CONTENT_HASH,
                    current_identity_key=f"{CASE_ID}|publication-notice-alt",
                )
            )
            ...
            _add_document_evidence(
                ...
                lineage_key="publication-notice-other-document",
            )
        elif relationship == "other_case":
            transaction.add(_case(case_id=OTHER_CASE_ID, case_no="V8-PUBLICATION-OTHER"))
            transaction.flush()
            _add_document_evidence(
                transaction,
                ...
                attachment_id=OTHER_ATTACHMENT_ID,
                evidence_version_id=OTHER_EVIDENCE_VERSION_ID,
                lineage_key="publication-notice-other-case",
            )
        else:
...
def test_route_dto_command_and_result_freeze_exact_public_contract() -> None:
    ...
    assert payload_type.model_validate(_request_data()).model_dump() == {
        ...
        "effective_at": EFFECTIVE_AT,
        "occurred_at": None,
        "idempotency_key": "publication-notice-1",
    }

    command_type = adapter.RecordPublicationNoticeCommand
    result_type = adapter.RecordPublicationNoticeResult
...
@pytest.mark.parametrize(
    ...
    (
        ...
        {**_request_data(), "event_type": "PUBLICATION_NOTICE_RECORDED"},
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("idempotency_key", StringSubclass("publication-notice-1")),
...
def test_valid_request_maps_only_publication_notice_and_commits_once(
    session_factory: sessionmaker,
    ...
    def apply(
        ...
        return LifecycleTransitionResult(
            case_id=CASE_ID,
            activity_id="activity-publication-api",
            sequence=1,
...
def test_cross_document_or_case_evidence_is_exact_400(
    ...
    assert response.json()["error"]["code"] == ("PUBLICATION_NOTICE_EVIDENCE_RELATION_MISMATCH")
...
def test_invalid_evidence_state_is_exact_409(
    ...
    assert response.json()["error"]["code"] == "PUBLICATION_NOTICE_EVIDENCE_CONFLICT"
    assert response.json()["error"]["details"] == {"field": "evidence_version_id"}
    _assert_case_unchanged(session_factory)
...
def test_authentication_and_doc_edit_permission_preserve_401_and_403(
    ...
    user_id = "publication-no-permission-user"
    with session_factory() as transaction:
        transaction.add(
            T_User(
                id=user_id,
                username="publication-no-permission",
                display_name="无权限用户",
...
def test_deeper_lifecycle_error_passes_through_and_rolls_back(
    ...
    def fail(command: LifecycleEventCommand, transaction: Session) -> None:
        ...
        assert case is not None
        case.official_procedure_stage = OfficialProcedureStage.PUBLISHED.value
        transaction.flush()
        raise BusinessError(
