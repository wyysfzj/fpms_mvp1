...
LINEAGE_KEY = "ic-layout-remuneration-adjudication-request-submission"
WRONG_LINEAGE_KEY = "ic-layout-nonvoluntary-license-request-submission"
REVIEWED_AT = datetime(2026, 7, 25, 10)
SUBMITTED_AT = datetime(2026, 7, 25, 10, 30)
...
def _boundary():
    command_type = getattr(
        fee_linking_service,
        "RecognizeIcLayoutRemunerationAdjudicationRequestObligationCommand",
        None,
    )
    recognize = getattr(
        fee_linking_service,
        "recognize_ic_layout_remuneration_adjudication_request_obligation",
        None,
    )
...
def _seed_case(transaction: Session) -> None:
    transaction.add(
        Case(
            id=CASE_ID,
            case_no="IC-LAYOUT-REMUNERATION-ADJUDICATION-REQUEST",
            case_type="NORMAL",
            patent_category="INV",
    ...
    *,
    ...
    transaction.add(
        DocAttachment(
            ...
            file_name=f"remuneration-adjudication-request-{ordinal}.xml",
            file_path=f"/evidence/remuneration-adjudication-request-{ordinal}.xml",
    ...
    *,
    ...
    result = evidence_workflow.finalize_external_submission(
        evidence_workflow.FinalizeExternalSubmissionCommand(
            ...
            idempotency_key=f"ic-layout-remuneration-adjudication-request-{ordinal}",
        ),
        transaction,
    )
    transaction.commit()
...
def _expect_conflict(callable_: object) -> None:
    ...
    assert captured.value.code == "IC_LAYOUT_REMUNERATION_ADJUDICATION_REQUEST_SOURCE_CONFLICT"
    ...
    try:
        ...
        assert {
            ...
            (
                CASE_ID,
                first_activity_id,
                "IC_LAYOUT_REMUNERATION_ADJUDICATION_REQUESTED",
                "CNY",
                "VERIFIED",
                ACTOR_ID,
            ),
...
@pytest.mark.parametrize(
    ...
    (
        ...
        "IC_LAYOUT_REMUNERATION_ADJUDICATION_REQUEST_INTENT_RECORDED",
    ),
)
