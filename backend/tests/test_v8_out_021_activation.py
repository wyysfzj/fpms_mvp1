...
from app.modules.documents.official_notice_catalog import (
    seed_official_letter_out_form_021_catalog as seed_doc_templates,
)
...
def test_seed_classifies_only_form_021_as_internal_reference_material(
    ...
    with session_factory() as transaction:
        ...
        for index, item in enumerate(rows[:21], start=1):
        ...
        row = rows[20]
        assert (row.code, row.name) == ("OFFICIAL_LETTER_OUT_021", "四通意见陈述")
        assert json.loads(row.input_fields or "{}") == {
            ...
            "legacy_form_scope": "form-021",
            "official_letter_name": "四通意见陈述",
