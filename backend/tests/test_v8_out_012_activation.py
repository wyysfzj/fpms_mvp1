...
from app.modules.documents.official_notice_catalog import (
    seed_official_letter_out_form_012_catalog as seed_doc_templates,
)
...
def test_seed_accumulates_form_012_internal_only_as_its_own_scope(
            ...
            transaction.execute(
                ...
                .where(
                    DocTemplate.code.in_(
                        tuple(f"OFFICIAL_LETTER_OUT_{index:03d}" for index in range(1, 13))
                    )
                )
                .order_by(DocTemplate.code.asc())
            )
