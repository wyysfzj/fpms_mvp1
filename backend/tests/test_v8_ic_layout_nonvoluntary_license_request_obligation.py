...
LINEAGE_KEY = "ic-layout-nonvoluntary-license-request-submission"
WRONG_LINEAGE_KEY = "ic-layout-extension-request-submission"
...
def _boundary():
    command_type = getattr(
        fee_linking_service,
        "RecognizeIcLayoutNonvoluntaryLicenseRequestObligationCommand",
        None,
    )
    recognize = getattr(
        fee_linking_service,
        "recognize_ic_layout_nonvoluntary_license_request_obligation",
        None,
    )
...
def _seed_case(transaction: Session) -> None:
    transaction.add(
        Case(
            id=CASE_ID,
            case_no="IC-LAYOUT-NONVOLUNTARY-LICENSE-REQUEST",
            case_type="NORMAL",
            patent_category="INV",
    ...
    *,
    ...
    transaction.add(
        DocAttachment(
            ...
            file_name=f"nonvoluntary-license-request-{ordinal}.xml",
            file_path=f"/evidence/nonvoluntary-license-request-{ordinal}.xml",
    ...
    *,
    ...
    result = evidence_workflow.finalize_external_submission(
        evidence_workflow.FinalizeExternalSubmissionCommand(
            ...
            idempotency_key=f"ic-layout-nonvoluntary-license-request-{ordinal}",
        ),
        transaction,
    )
    transaction.commit()
...
def _expect_conflict(callable_: object) -> None:
    ...
    assert captured.value.code == "IC_LAYOUT_NONVOLUNTARY_LICENSE_REQUEST_SOURCE_CONFLICT"
    ...
    try:
        ...
        assert {
            ...
            (
                CASE_ID,
                first_activity_id,
                "IC_LAYOUT_NONVOLUNTARY_LICENSE_REQUESTED",
                "CNY",
                "VERIFIED",
                ACTOR_ID,
            ),
...
@pytest.mark.parametrize(
    ...
    (
        ...
        "IC_LAYOUT_NONVOLUNTARY_LICENSE_REQUEST_INTENT_RECORDED",
    ),
)
