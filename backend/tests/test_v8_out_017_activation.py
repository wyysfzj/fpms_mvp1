...
from app.modules.documents.official_notice_catalog import (
    seed_official_letter_out_form_017_catalog as seed_doc_templates,
)
...
def test_seed_accumulates_form_017_internal_only_as_its_own_scope(
    ...
    with session_factory() as transaction:
        ...
        assert [(row.code, row.name) for row in classified] == [
            ...
            ("OFFICIAL_LETTER_OUT_014", "费用减缓请求书"),
            ("OFFICIAL_LETTER_OUT_015", "改正译文错误请求书"),
            ("OFFICIAL_LETTER_OUT_016", "PPH请求"),
            ("OFFICIAL_LETTER_OUT_017", "发明主动修改"),
        ]
        for index, row in enumerate(classified, start=1):
            payload = json.loads(row.input_fields or "{}")
            assert payload == _classified_payload(row.name, f"form-{index:03d}")
        ...
        assert json.loads(classified[8].input_fields or "{}")["legacy_form_scope"] == "form-009"
        assert json.loads(classified[16].input_fields or "{}")["legacy_form_scope"] == "form-017"
        for row in rows[17:]:
            ...
            transaction.execute(
                ...
                .where(
                    DocTemplate.code.in_(
                        tuple(f"OFFICIAL_LETTER_OUT_{index:03d}" for index in range(1, 18))
                    )
                )
                .order_by(DocTemplate.code.asc())
            )
