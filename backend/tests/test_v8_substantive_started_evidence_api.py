...
PATH = "/api/v1/documents/{document_id}/lifecycle/substantive-start"
ROUTER_PATH = "/documents/{document_id}/lifecycle/substantive-start"
CASE_ID = "case-substantive-api"
DOCUMENT_ID = "document-substantive-api"
ATTACHMENT_ID = "attachment-substantive-api"
EVIDENCE_VERSION_ID = "evidence-substantive-api"
ALT_ATTACHMENT_ID = "attachment-substantive-alt"
ALT_EVIDENCE_VERSION_ID = "evidence-substantive-alt"
OTHER_CASE_ID = "case-substantive-other"
OTHER_DOCUMENT_ID = "document-substantive-other"
OTHER_ATTACHMENT_ID = "attachment-substantive-other"
OTHER_EVIDENCE_VERSION_ID = "evidence-substantive-other"
ACTOR_USERNAME = "admin"
CREATOR_ID = "substantive-creator"
REVIEWER_ID = "substantive-reviewer"
...
def _request_data(
    *,
    ...
    idempotency_key: str = "substantive-start-1",
) -> dict[str, object]:
...
def _case(
    *,
    case_id: str = CASE_ID,
    case_no: str = "V8-SUBSTANTIVE-API",
) -> Case:
    return Case(
        id=case_id,
        case_no=case_no,
        ...
        title_cn="实质审查开始证据 API 测试案件",
        status="PUBLISHED",
        business_stage=BusinessStage.PROSECUTION_MANAGEMENT.value,
        official_procedure_stage=OfficialProcedureStage.PUBLISHED.value,
        ...
        if relationship == "same_document":
            transaction.add(
                DocAttachment(
                    ...
                    document_id=DOCUMENT_ID,
                    file_name="substantive-alt.pdf",
                    file_path="/evidence/substantive-alt.pdf",
                    mime_type="application/pdf",
                    content_hash=CONTENT_HASH,
                )
            )
            ...
            transaction.add(
                DocumentEvidenceVersion(
                    ...
                    lineage_key="substantive-start-alt",
                    role="OFFICIAL_FINAL_PDF",
                    version_number=1,
                    state="FINAL",
                    creator_id=CREATOR_ID,
                    ...
                    reviewed_at=REVIEWED_AT,
                    content_hash=CONTENT_HASH,
                    current_identity_key=f"{CASE_ID}|substantive-start-alt",
                )
            )
            ...
            _add_document_evidence(
                ...
                lineage_key="substantive-start-other-document",
            )
        elif relationship == "other_case":
            transaction.add(_case(case_id=OTHER_CASE_ID, case_no="V8-SUBSTANTIVE-OTHER"))
            transaction.flush()
            _add_document_evidence(
                transaction,
                ...
                attachment_id=OTHER_ATTACHMENT_ID,
                evidence_version_id=OTHER_EVIDENCE_VERSION_ID,
                lineage_key="substantive-start-other-case",
            )
        else:
...
def test_route_dto_command_and_result_freeze_exact_public_contract() -> None:
    ...
    assert payload_type.model_validate(_request_data()).model_dump() == {
        ...
        "effective_at": EFFECTIVE_AT,
        "occurred_at": None,
        "idempotency_key": "substantive-start-1",
    }

    command_type = adapter.StartSubstantiveExaminationCommand
    result_type = adapter.StartSubstantiveExaminationResult
...
@pytest.mark.parametrize(
    ...
    (
        ...
        {**_request_data(), "event_type": "SUBSTANTIVE_EXAMINATION_STARTED"},
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("idempotency_key", StringSubclass("substantive-start-1")),
...
def test_valid_request_maps_only_substantive_started_and_commits_once(
    session_factory: sessionmaker,
    ...
    def apply(
        ...
        return LifecycleTransitionResult(
            case_id=CASE_ID,
            activity_id="activity-substantive-api",
            sequence=1,
            lifecycle_revision=1,
            lane=ActivityLane.LIFECYCLE,
            event_type="SUBSTANTIVE_EXAMINATION_STARTED",
            confirmation_status=ConfirmationStatus.CONFIRMED,
            previous_projection=PUBLISHED_PROJECTION,
            current_projection=SUBSTANTIVE_PROJECTION,
            legacy_case_status="SUB_EXAM",
...
def test_cross_document_or_case_evidence_is_exact_400(
    ...
    assert response.json()["error"]["code"] == ("SUBSTANTIVE_START_EVIDENCE_RELATION_MISMATCH")
...
def test_invalid_evidence_state_is_exact_409(
    ...
    assert response.json()["error"]["code"] == "SUBSTANTIVE_START_EVIDENCE_CONFLICT"
    assert response.json()["error"]["details"] == {"field": "evidence_version_id"}
    _assert_case_unchanged(session_factory)
...
def test_authentication_and_doc_edit_permission_preserve_401_and_403(
    ...
    user_id = "substantive-no-permission-user"
    with session_factory() as transaction:
        transaction.add(
            T_User(
                id=user_id,
                username="substantive-no-permission",
                display_name="无权限用户",
...
def test_deeper_lifecycle_error_passes_through_and_rolls_back(
    ...
    def fail(command: LifecycleEventCommand, transaction: Session) -> None:
        ...
        assert case is not None
        case.official_procedure_stage = OfficialProcedureStage.SUBSTANTIVE_EXAMINATION.value
        transaction.flush()
        raise BusinessError(
