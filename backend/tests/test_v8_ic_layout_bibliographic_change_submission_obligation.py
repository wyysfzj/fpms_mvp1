...
LINEAGE_KEY = "ic-layout-bibliographic-change-submission"
WRONG_LINEAGE_KEY = "ic-layout-right-restoration-request-submission"
...
def _boundary():
    command_type = getattr(
        fee_linking_service,
        "RecognizeIcLayoutBibliographicChangeSubmissionObligationCommand",
        None,
    )
    recognize = getattr(
        fee_linking_service,
        "recognize_ic_layout_bibliographic_change_submission_obligation",
        None,
    )
...
def _seed_case(transaction: Session) -> None:
    transaction.add(
        Case(
            id=CASE_ID,
            case_no="IC-LAYOUT-BIBLIOGRAPHIC-CHANGE",
            case_type="NORMAL",
            patent_category="INV",
    ...
    *,
    ...
    transaction.add(
        DocAttachment(
            ...
            file_name=f"bibliographic-change-{ordinal}.xml",
            file_path=f"/evidence/bibliographic-change-{ordinal}.xml",
    ...
    *,
    ...
    result = evidence_workflow.finalize_external_submission(
        evidence_workflow.FinalizeExternalSubmissionCommand(
            ...
            idempotency_key=f"ic-layout-bibliographic-change-submission-{ordinal}",
        ),
        transaction,
    )
    transaction.commit()
...
def _command(source_activity_id: str, evidence_id: str):
...
def _expect_conflict(callable_: object) -> None:
    ...
    assert captured.value.code == "IC_LAYOUT_BIBLIO_CHANGE_SOURCE_CONFLICT"


def test_each_reviewed_final_submission_forms_and_reuses_only_its_own_fee(
    ...
    try:
        ...
        assert {
            ...
            (
                CASE_ID,
                first_activity_id,
                "IC_LAYOUT_BIBLIO_CHANGE_SUBMITTED",
                "CNY",
                "VERIFIED",
                ACTOR_ID,
            ),
        ...
        assert {
            ...
            (
                "IC_LAYOUT_BIBLIOGRAPHIC_CHANGE_FEE",
                "布图设计著录事项变更手续费",
                0,
                Decimal("50.00"),
                Decimal("0.0000"),
...
@pytest.mark.parametrize(
    "source_defect", ("wrong_lineage", "missing_reference", "multiple_reference")
)
def test_wrong_or_nonunique_submission_lineage_is_409_with_zero_fee_write(
    session_factory: sessionmaker[Session],
    source_defect: str,
) -> None:
    transaction = session_factory()
    try:
        ...
        evidence_id = _seed_evidence(
            transaction,
            ordinal=1,
            lineage_key=WRONG_LINEAGE_KEY if source_defect == "wrong_lineage" else LINEAGE_KEY,
        )
        ...
        if source_defect == "missing_reference":
...
@pytest.mark.parametrize("field", ("current_identity_key", "state", "reviewer_id"))
def test_noncurrent_nonfinal_or_nonindependent_source_is_409_with_zero_fee_write(
    session_factory: sessionmaker[Session],
    field: str,
) -> None:
    transaction = session_factory()
