...
_DEMO_GRANT_OFFICIAL_SOURCE_EVENT_TYPE = "DEMO_GRANT_OFFICIAL_FEE_CONFIRMED"
_DEMO_GRANT_OFFICIAL_SOURCE_SCHEMA = "FPMS_DEMO_GRANT_OFFICIAL_FEE_CONFIRMED_V1"
_DEMO_GRANT_OFFICIAL_SOURCE_PAYLOAD_KEYS = {
    "schema",
    "grant_fee_task_id",
    "case_id",
    "source_document_id",
...
def record_grant_fee_task_instruction(
    ...
    with transaction.no_autoflush:
        ...
        if activity is None:
            _grant_instruction_link_not_found()
        if activity.activity_type == _DEMO_GRANT_OFFICIAL_SOURCE_EVENT_TYPE:
            if command.instruction != FeeClientInstruction.PAY.value:
                _grant_instruction_lineage_conflict()
            obligation_id = _grant_registration_instruction_obligation(
                transaction,
                task=task,
                activity=activity,
            )
            ...
            try:
                ...
                if exc.code != "GRANT_INSTRUCTION_LINEAGE_CONFLICT":
                    raise
                try:
                    reviewed_obligation, _review = (
                        validated_grant_year_official_fee_review_for_draft(
                            transaction,
                    ...
                    _grant_instruction_lineage_conflict()
                if reviewed_obligation.id != obligation_id:
                    _grant_instruction_lineage_conflict()
...
def _demo_grant_official_source_context(
    transaction: Session,
    *,
    task: T_GrantFeeTask,
    activity: CaseActivityEvent,
    ...
    if (
        ...
        or payload["schema"] != _DEMO_GRANT_OFFICIAL_SOURCE_SCHEMA
        or payload["grant_fee_task_id"] != task.id
        or payload["case_id"] != task.case_id
        or payload["source_document_id"] != task.source_document_id
        or type(payload["reviewed_evidence_version_id"]) is not str
        or type(payload["reviewed_evidence_content_hash"]) is not str
        or _GRANT_NOTICE_HASH_PATTERN.fullmatch(
            payload["reviewed_evidence_content_hash"]
        )
        is None
        or type(payload["preview_digest"]) is not str
        or _GRANT_NOTICE_HASH_PATTERN.fullmatch(payload["preview_digest"]) is None
        or type(payload["rate_book_version"]) is not str
        or not payload["rate_book_version"]
        or type(payload["rate_book_sha256"]) is not str
        or _GRANT_NOTICE_SNAPSHOT_HASH_PATTERN.fullmatch(
            payload["rate_book_sha256"]
        )
        is None
        ...
        or activity.case_id != task.case_id
        or activity.activity_type != _DEMO_GRANT_OFFICIAL_SOURCE_EVENT_TYPE
    ...
    for line in lines:
        if (
            ...
            or set(line) != {"fee_code", "quantity", "confirmed_payable_amount"}
            or not _valid_review_text(line.get("fee_code"), limit=64)
            or line["fee_code"] in seen_codes
            or line.get("quantity") != 1
            or type(line.get("confirmed_payable_amount")) is not str
            or _GRANT_NOTICE_CANONICAL_AMOUNT_PATTERN.fullmatch(
                line["confirmed_payable_amount"]
            )
            is None
        ):
            _grant_review_lineage_conflict()
        seen_codes.add(line["fee_code"])
    case = transaction.get(Case, task.case_id)
    document = transaction.get(Document, task.source_document_id)
    ...
    if len(stored_refs) != 2 or stored_signatures != expected_signatures:
        _grant_review_lineage_conflict()
    return case, payload, expected_refs


def _grant_registration_review_context(
    ...
    *,
    ...
    case, payload, evidence_refs = _demo_grant_official_source_context(
        transaction,
        task=task,
        activity=activity,
    )
    ...
    obligation = transaction.get(FeeObligation, command.obligation_id)
    if named_evidence is None or obligation is None:
        _grant_review_link_not_found()
    if (
        command.reviewed_evidence_version_id
        != payload["reviewed_evidence_version_id"]
        or command.expected_content_hash
        != payload["reviewed_evidence_content_hash"]
    ):
        _grant_review_lineage_conflict()
    obligations = tuple(
    ...
    if len(obligations) != 1 or obligations[0].id != obligation.id:
        _grant_review_lineage_conflict()
    lines = tuple(
        transaction.scalars(
    ...
    source_lines = {line["fee_code"]: line for line in payload["lines"]}
    if (
        _grant_instruction_recognition_count(
            transaction,
            case_id=task.case_id,
            obligation_id=obligation.id,
        )
        != 1
        or obligation.case_id != task.case_id
        ...
        or obligation.obligation_type != "GRANT_REGISTRATION_OFFICIAL_FEES"
        or obligation.source_status != FeeSourceStatus.VERIFIED.value
        or obligation.currency != "CNY"
        or obligation.due_date != task.due_date
        or len(lines) != len(source_lines)
        or {line.fee_code for line in lines} != set(source_lines)
    ):
        _grant_review_lineage_conflict()
    for line in lines:
        expected_identity = hashlib.sha256(
            f"{task.case_id}|{activity.id}|{line.fee_code}|{line.fee_year_key}".encode()
        ).hexdigest()
        if (
            line.case_id != task.case_id
            or line.source_activity_id != activity.id
            or line.fee_year_key != 0
            or line.reduction_ratio != Decimal("0.0000")
            or line.payable_amount
            != Decimal(source_lines[line.fee_code]["confirmed_payable_amount"])
            or line.source_amount != line.payable_amount
            or line.source_date is None
...
def _grant_registration_instruction_obligation(
    transaction: Session,
    *,
    task: T_GrantFeeTask,
    activity: CaseActivityEvent,
) -> str:
    _case, payload, _evidence_refs = _demo_grant_official_source_context(
        transaction,
        task=task,
        activity=activity,
    )
    ...
    source_lines = {line["fee_code"]: line for line in payload["lines"]}
    if (
        detail.case_id != task.case_id
        or detail.source.source_activity_id != activity.id
        or detail.source.source_document_id != task.source_document_id
        or detail.source.status is not FeeSourceStatus.VERIFIED
        or detail.fee_domain is not FeeDomain.GOV
        or detail.obligation_type != "GRANT_REGISTRATION_OFFICIAL_FEES"
        or detail.due_date != task.due_date
        or detail.currency != "CNY"
        or len(detail.lines) != len(source_lines)
        or {line.fee_code for line in detail.lines} != set(source_lines)
    ):
        _grant_instruction_lineage_conflict()
    for line in detail.lines:
        amount = Decimal(source_lines[line.fee_code]["confirmed_payable_amount"])
        if (
            line.fee_year_key != 0
            or line.official_full_amount != amount
            or line.reduction_ratio != Decimal("0.0000")
            or line.payable_amount != amount
            or line.source_amount != amount
            or line.difference_review_state is not FeeDifferenceReviewState.MATCHED
        ):
            _grant_instruction_lineage_conflict()
    reviews = _matching_grant_review_activities(
        transaction,
        case_id=task.case_id,
        grant_fee_task_id=task.id,
        obligation_id=obligation.id,
...
def _grant_review_context(
    ...
    if activity is None:
        _grant_review_link_not_found()
    if activity.activity_type == _DEMO_GRANT_OFFICIAL_SOURCE_EVENT_TYPE:
        return _grant_registration_review_context(
            command,
            transaction,
            task=task,
            activity=activity,
    ...
    named_line_ids = {
        ...
        for line_id in transaction.scalars(
            select(FeeObligationLine.id).where(
                FeeObligationLine.id.in_(tuple(line.obligation_line_id for line in command.lines))
            )
        )
    }
    if (
    ...
    try:
        ...
        if exc.code in {"FEE_OBLIGATION_NOT_FOUND"}:
            _grant_review_link_not_found()
        if exc.code != "FEE_OBLIGATION_STORED_STATE_INVALID" or not (
            _grant_review_exact_unpaid_draft(
                transaction, case=case, obligation=obligation, lines=lines
            )
        ):
            _grant_review_lineage_conflict()
    else:
...
def _grant_review_exact_unpaid_draft(
    transaction: Session,
    *,
    case: Case,
    ...
    if obligation.draft_status != "CREATED":
        return False
    line_by_id = {line.id: line for line in lines}
    links = tuple(
        transaction.scalars(
            select(FeeObligationDraftItemLink).where(
                FeeObligationDraftItemLink.obligation_line_id.in_(tuple(line_by_id))
            )
        )
    )
    if len(links) != len(lines) or {link.obligation_line_id for link in links} != set(line_by_id):
        return False
    item_ids = tuple(link.fee_item_id for link in links)
    items = {
        item.id: item
        for item in transaction.scalars(select(FeeItem).where(FeeItem.id.in_(item_ids)))
    }
    if set(items) != set(item_ids):
        return False
    draft_ids = {item.draft_id for item in items.values()}
    if len(draft_ids) != 1:
        return False
    draft = transaction.get(FeeDraft, next(iter(draft_ids)))
    if (
        draft is None
        or draft.case_id != case.id
        or draft.client_id != case.client_id
        or draft.currency != obligation.currency
        or draft.draft_type != "GENERIC"
        or transaction.scalar(
            select(func.count()).select_from(GovPayment).where(GovPayment.fee_item_id.in_(item_ids))
        )
        != 0
    ):
        return False
    for link in links:
        line = line_by_id[link.obligation_line_id]
        item = items[link.fee_item_id]
        if (
            item.draft_id != draft.id
            or item.case_id != case.id
            or item.fee_code != line.fee_code
            or item.fee_name != line.fee_name
            or item.fee_type != FeeDomain.GOV.value
    ...
    *,
    ...
    try:
        previous_projection = LifecycleProjection(
            ...
            legal_status=(
                None
                if existing.old_legal_status is None
                else LegalStatus(existing.old_legal_status)
            ),
        ...
        current_projection = LifecycleProjection(
            ...
            legal_status=(
                None
                if existing.new_legal_status is None
                else LegalStatus(existing.new_legal_status)
            ),
            lifecycle_verification_status=None,
        )
    except ValueError:
    ...
    with transaction.no_autoflush:
        ...
        for line in lines:
            predicates = (
                ...
                cast(FeeObligationLine.updated_at, String) == line.updated_at.isoformat(sep=" "),
...
def _grant_draft_command_invalid(field: str) -> None:
    raise_business_error(
        "GRANT_DRAFT_COMMAND_INVALID",
        "授权费用草单命令无效",
        details={"field": field},
        status_code=400,
    )
...
def _grant_draft_link_not_found() -> None:
    raise_business_error(
...
def _grant_draft_lineage_conflict() -> None:
    raise_business_error(
...
def _validate_grant_draft_command(command: object) -> None:
    if type(command) is not PrepareGrantFeeTaskDraftCommand:
        _grant_draft_command_invalid("command")
    for field, limit in (
        ("grant_fee_task_id", 36),
        ("source_activity_id", 36),
        ("actor_id", 36),
...
def _validate_grant_draft_result(
    ...
    *,
    ...
    if (
        ...
        or type(result.links) is not tuple
        or not result.links
        or any(type(link) is not FeeDraftItemLinkResult for link in result.links)
    ):
        _grant_draft_lineage_conflict()
    line_ids = tuple(line.id for line in obligation_lines)
    if (
        tuple(link.obligation_line_id for link in result.links) != line_ids
        or len({link.id for link in result.links}) != len(result.links)
        or len({link.fee_item_id for link in result.links}) != len(result.links)
        or any(link.reused is not result.activity_reused for link in result.links)
    ):
        _grant_draft_lineage_conflict()
    ...
    if (
        ...
        or draft.currency != obligation.currency
        or draft.draft_type != "GENERIC"
        or len(persisted_links) != len(result.links)
        or set(persisted_items) != {link.fee_item_id for link in result.links}
    ):
        _grant_draft_lineage_conflict()
    expected_link_identity = {
        (link.id, link.obligation_line_id, link.fee_item_id) for link in result.links
    }
    if {
        (link.id, link.obligation_line_id, link.fee_item_id) for link in persisted_links
    } != expected_link_identity:
        _grant_draft_lineage_conflict()
    line_by_id = {line.id: line for line in obligation_lines}
    for link in result.links:
        line = line_by_id.get(link.obligation_line_id)
        item = persisted_items[link.fee_item_id]
        if (
            line is None
            or item.draft_id != draft.id
            or item.case_id != task.case_id
            or item.fee_code != line.fee_code
            or item.fee_name != line.fee_name
            or item.fee_type != FeeDomain.GOV.value
            or item.year_no != line.fee_year_key
    ...
    if (
        ...
        _grant_draft_lineage_conflict()
    obligation_model = transaction.get(FeeObligation, obligation.id)
    if (
...
def prepare_grant_fee_task_draft(
    ...
    _validate_grant_draft_command(command)
    if not isinstance(transaction, Session):
    ...
    with transaction.no_autoflush:
        ...
        if task is None:
            raise_business_error(
                "GRANT_DRAFT_TASK_NOT_FOUND",
                "授权费用任务不存在",
                status_code=404,
            )
        source_activity = transaction.get(CaseActivityEvent, command.source_activity_id)
        if source_activity is None:
            _grant_draft_link_not_found()
        try:
            _case, _expected_lines, _payload = _grant_instruction_expected_source(
                transaction,
                task=task,
        ...
        if obligation.source_activity_id != source_activity.id or not obligation_lines:
            _grant_draft_lineage_conflict()
