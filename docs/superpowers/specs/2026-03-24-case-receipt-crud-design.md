...
**CaseReceiptResponse** (updated): Add `fee_name`, `due_date`, `is_prepayment`, `remark` to existing schema.

**CaseReceiptListItem**: Extends CaseReceiptResponse with `case_no: str | None` and `client_name: str | None` (joined).

**CaseReceiptListResponse**: `{ items: list[CaseReceiptListItem], total: int }`
...
- **V-CR-02**: If `received_amt > receivable_amt` and `is_prepayment` not explicitly set → server sets `is_prepayment = True`
- **V-CR-03**: If `received_amt < receivable_amt` and `is_arrears` not explicitly set → server sets `is_arrears = True`
...
### `create_case_receipt(db, payload, current_user) → CaseReceipt`

1. Validate case_id exists (404 if not)
2. Apply V-CR-02/V-CR-03 defaults for `is_arrears` / `is_prepayment` when not explicitly provided
3. Generate UUID in app code
4. Set `created_by` / `updated_by` from current_user
5. Return created record

### `update_case_receipt(db, receipt_id, payload, current_user) → CaseReceipt`

1. Find receipt by id (404 if not)
2. Apply partial update (only non-None fields from `CaseReceiptUpdate`)
3. Recompute V-CR-02/V-CR-03 defaults if amounts changed and flags not explicitly provided in this request
4. Set `updated_by` from current_user
5. Return updated record

### `list_case_receipts(db, filters) → (list[CaseReceiptListItem], total)`

1. Join `t_case_receipt` → `t_case` → `t_client` for `case_no` / `client_name`
...
5. Order by `last_receipt_date DESC NULLS LAST`, then `created_at DESC`
...
| `backend/tests/test_case_receipt_crud.py` | NEW — 16 tests |
