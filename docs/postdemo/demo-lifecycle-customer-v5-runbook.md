...
1. 主持人在不共享的控制窗口直接打开 `/demo/abc`，复核 SERVICE rate provenance，并创建唯一 SERVICE obligation；
2. 立即切回客户可见的正常案件详情页，打开“费用”页签，查看刚创建的服务费义务；
...
> 如需调用本地演示专用的账单、回款或核销动作，主持人只在不共享的 `/demo/abc` 控制窗口触发；
> 客户共享屏幕始终停留在正常的草单、账单、回款、核销和案件页面，并在每次动作后复核权威结果。
