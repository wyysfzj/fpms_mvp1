...
客户 2026-06-11 补充答复已关闭三项关键待确认：总委号是申请人/客户级“总委托书备案编号”且同一申请人唯一；复杂 OA 意见陈述转 PDF 后在官方“附加文件”中按“其他证明文件”提交；旧系统 `0 / 0.7 / 0.85` 费减值是“减免比例”，必须转换为应缴比例后才能进入计费。
...
- 不把客户旧系统费减值直接写入现有 `fee_reduction` 计费乘数字段；客户已确认旧值是“减免比例”，P1 必须显式转换为应缴比例或拆分字段。
...
- 技术交底书。
- 委托指示（如有）。
- 代理人、撰稿人、代理助理、总委托书备案编号、费减比例、页数/项数/幅数。
...
- 总委号归属已确认：它是申请人/客户级“总委托书备案编号”，同一申请人唯一，案件递交包应引用该申请人/客户官方资料字段。若 FPMS 的 `Client` 与官方 `Applicant` 不一致，P1 需保留映射规则待确认。
- 费减比例语义已确认：客户旧系统 `0 / 0.7 / 0.85` 是“减免比例”，不是当前申请费生成逻辑中的应缴乘数。P1 需显示客户减免比例，并计算应缴比例。
- 代理人资格证号、代理助理等仍未确认最终维护来源的字段，FS 必须先定义候选维护位置和校验规则，并标记待客户确认最终归属。
...
5. 检查发明名称、代理人、撰稿人、代理助理、总委托书备案编号、页数、权利要求项数、附图幅数、费减比例。
...
- 可复制纯文本意见。
- PDF 保真附件。
- 修改后的权利要求书、说明书修改页、修改对照页、其他证明文件；复杂意见陈述转 PDF 后按“附加文件 -> 其他证明文件”处理。
- “补交了实验数据”条件标志。
- 官方页面操作 checklist。
...
| 总委托书备案编号 | 客户/申请人官方资料 enhancement | 按申请人/客户带出并在递交包校验 | 客户已确认同一申请人唯一、案件复用；当前 `Client` / `Applicant` 未见稳定字段，客户与官方申请人不一致时映射待确认 |
| 页数、项数、幅数 | `spec_pages`、`draw_pages`、`claim_count`、`claim_pages` | 自动带出并校验非空 | 序列表页数/文件角色待确认 |
| 费减比例 | 客户旧系统减免比例；当前 `fee_reduction`/`discount_rate` 需重新映射 | 显示客户减免比例，并计算官方应缴比例 | 已确认 `0 / 0.7 / 0.85` 是减免比例；当前申请费生成使用应缴乘数，不能直接复用旧值 |
...
| 规格/费减 | `spec_pages`、`draw_pages`、`claim_count`、`claim_pages`、`has_exam_request`、`fee_reduction`、`discount_rate` | `Case` model；`PRODUCT-A-CASE-SPEC-FEE-DISCOUNT-CONTRACT-01.md`；客户答复确认旧系统费减值为减免比例 |
...
| 客户总委号 / 总委托书备案编号 | 官方请求书 / 证明文件备案 | 已确认是申请人/客户级字段，同一申请人唯一，案件复用；P1 应新增到客户/申请人官方资料或客户-申请人映射，不作为长期提交前补录 |
| 客户费减比例 | 官费计算 / 官方缴费提示 | 已确认是减免比例；P1 应保存或展示客户减免比例，并生成应缴比例 `1 - 减免比例` 供当前计费逻辑使用 |
...
| 证明文件备案编号 | 证明文件备案 / 总委托书备案编号候选入口 | 客户已解释总委号来源；官方页面“证明文件备案编号”是否一律填写该总委托书备案编号仍需按页面/样例确认 |
...
| PDF 附件类别 | OA 答复官方页面 | 已确认：复杂意见陈述 PDF 在官方“附加文件”中选择“其他证明文件” |
| 格式函联系人来源 | 信函抬头 | 待客户确认 |
...
- OA 答复 PDF 附件的官方类别已确认为“附加文件 -> 其他证明文件”；统一引用话术是否固定为“意见陈述正文请见其他证明文件”仍待确认。
- OA 答复中“补交了实验数据”勾选项的适用规则。
- 回执 PDF 元数据和收到文件清单是否需要人工录入、OCR 提取，还是仅在上传后人工查看。

已由 `相关问题解答.docx` 关闭、不得继续作为“待确认”的内容：

- 总委号层级：申请人/客户级“总委托书备案编号”，同一申请人唯一，关联案件共用。
- 费减语义：`0 / 0.7 / 0.85` 是减免比例；计费应使用应缴比例 `1 - 减免比例`。
- 复杂 OA 意见陈述 PDF 的官方附件类别：附加文件中的“其他证明文件”。
...
| PDF 意见陈述 | 格式保真附件 | Word 转 PDF 或人工上传 | 已确认按官方“附加文件 -> 其他证明文件”提交，通常承载无法正常复制显示的意见陈述书附页/正文内容 |
| 其他证明文件 | OA 答复附件 / PDF 保真承载 | 人工上传或系统生成 | 标记官方页面类别、文件说明和上传位置；复杂意见陈述 PDF 使用该类别 |
...
- 代理人、撰稿人、代理助理、代理资格证号、总委托书备案编号，并显示字段维护入口。
- 页数、项数、幅数、客户减免比例和系统应缴比例。
...
- 意见陈述 Word、纯文本意见、PDF 保真附件；当 PDF 承载复杂格式时，官方附件类别固定为“其他证明文件”。
...
| AC-06 | OA 答复工作包能组织 Word 意见陈述、纯文本意见、PDF 保真附件、修改文件、附加文件和官方页面 checklist；复杂意见陈述 PDF 明确标记为官方“附加文件 -> 其他证明文件”。 |
...
| AC-17 | 总委托书备案编号按申请人/客户级维护并可被案件递交准备包复用；同一申请人不得被设计为多个长期总委号，客户/申请人不一致时必须显示映射待确认。 |
| AC-18 | 费减处理区分客户“减免比例”和当前计费“应缴比例”：`0.85` 减免必须转换为 `0.15` 应缴，`0.7` 转换为 `0.3`，无费减应缴比例为 `1.0`。 |
...
1. 已答复：客户总委号是申请人/客户级“总委托书备案编号”，同一申请人有且只有一个，关联案件共用；若 FPMS 客户与官方申请人不是同一对象，映射维护位置仍需确认。
...
3. 已答复：PDF 保真附件在官方页面“附加文件”中选择“其他证明文件”。
...
1. 已答复：客户旧系统中的费减比例 `0 / 0.7 / 0.85` 表示减免比例；计费应转换为实际缴纳比例。
...
2. “陈述的意见”栏引用 PDF 附件的标准话术是否固定为截图中的“意见陈述正文请见其他证明文件”，还是不同 OA 场景需要不同模板？
...
| GAP | 证据 |
| --- | --- |
| 申请人/发明人官方字段已部分实现，主数据仍需区分 | 当前 `T_CaseApplicant` 已有 `nationality`、`certificate_type`、`certificate_no`、`official_postcode`、`official_applicant_kind`；`T_CaseInventor` 已有 `nationality`、`china_id_no`；前端 API 类型也已暴露这些字段。剩余 GAP 是申请人主数据 `Applicant` 仍未承载这些官方资料，总委托书备案编号也未见稳定字段。 |
...
| 附件官方提交角色已部分实现，PDF 生成/类别规则需固化 | 当前 `DocAttachment` 已有 `official_file_role`、`external_upload_position`，并已有 `OfficialWorkPackageManifest`；`official_workflows` 已定义 `OA_STATEMENT_PDF`、`OA_OTHER_PROOF` 等角色。剩余 GAP 是复杂意见陈述 PDF 的生成、引用话术和上传类别需要按客户答复固化为“附加文件 -> 其他证明文件”，且不等于自动上传。 |
...
| 客户费减比例与当前计费字段语义冲突 | 客户答复确认 `0 / 0.7 / 0.85` 是减免比例；当前申请费生成把 `case.fee_reduction` 作为 GOV 费用乘数，产品合同示例 `0.15` 表示缴纳 15%。P1 需要新增转换规则或拆分字段，不得直接复用旧值。 |
| 总委托书备案编号缺承载字段 | 客户答复确认总委号是申请人/客户级且同一申请人唯一；当前 `Client`、`Applicant`、`Case`、`T_CaseApplicant` 未见总委号字段。P1 需要申请人/客户官方资料或映射字段。 |
...
- 客户总委号：已确认是申请人/客户级“总委托书备案编号”，不再作为提交前补录或归属待确认项；代理人资格证号、代理助理仍需定义候选维护对象和确认问题。
...
- `证明文件备案编号` 是截图中的官方字段入口；客户已解释总委号为“总委托书备案编号”，P1 应按申请人/客户官方资料维护总委号，并用样例确认该字段在官方页面中的具体填报位置。
...
- OA 附加文件角色在截图中至少包括权利要求书、修改对照页、其他证明文件。客户答复已确认：复杂意见陈述 PDF 应作为“其他证明文件”上传；引用话术是否固定为截图示例仍需客户确认。
...
本节吸收 `/Users/cfcc/Documents/相关问题解答.docx` 的 2026-06-11 客户答复，用于修正 P1 开发和测试输入。

### 18.1 已关闭的规则

- 总委号：正式名称为“总委托书备案编号”，属于申请人/客户级官方编号；申请人/客户不是必须有，但同一申请人有且只有一个，关联案件复用同一编号。P1 应按申请人/客户官方资料维护，案件递交准备包只引用和校验。
- OA PDF 保真附件：意见陈述如能完整复制并正常显示，则直接复制到官方“陈述的意见”；如公式、符号、图片、表格等无法正常显示，则将整个意见陈述文件转 PDF，将“意见陈述书附页”作为官方“附加文件 -> 其他证明文件”提交。
- 费减比例：旧系统 `0 / 0.7 / 0.85` 表示“减免比例”，不是“实际缴纳比例”。`0.85` 表示减免 85%，应缴比例为 `0.15`；`0.7` 表示减免 70%，应缴比例为 `0.3`；无费减展示为 `0` 减免，计费应缴比例为 `1.0`。

### 18.2 当前实现影响

- 当前 `T_CaseApplicant` / `T_CaseInventor` 已有部分官方字段，FS 后续任务应避免重复设计这些字段；剩余重点是申请人/客户主数据沉淀、总委托书备案编号和客户/申请人映射。
- 当前 `official_workflows` 和附件 manifest 已有 `OA_STATEMENT_PDF`、`OA_OTHER_PROOF` 等角色，后续实现/测试应校验复杂意见陈述 PDF 被归入“其他证明文件”，但不能把这等同于自动上传官方页面。
- 当前申请费生成使用 `case.fee_reduction` 作为 GOV 费用乘数，和客户旧系统“减免比例”语义相反。后续开发必须新增转换层或拆分字段，测试应覆盖 `0.85 -> 0.15`、`0.7 -> 0.3`、无费减 -> `1.0`。

### 18.3 仍待确认

- 客户与官方申请人不是同一对象时，总委托书备案编号维护在 `Client`、`Applicant` 还是客户-申请人映射表。
- “意见陈述正文请见其他证明文件”是否作为统一话术模板，或不同 OA/补正/复审场景需要不同表达。
- 代理人资格证号来源、代理助理影响范围、回执 override 权限、官网缴费 Excel 样例、CPC/专利业务办理系统客户端导入范围、龙虾交接格式。
