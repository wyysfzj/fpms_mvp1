...
def _wait_http(process: subprocess.Popen[str], url: str, log_path: Path) -> None:
    ...
    for _ in range(160):
        if process.poll() is not None:
            raise RuntimeError(
                f"child exited rc={process.returncode}: "
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="fpms-row283-paylist-") as temp_name:
        temp = Path(temp_name)
            ...
            try:
                ...
                return subprocess.run(
                    [
                        ...
                        "playwright",
                        "test",
                        "src/tests/v8-pay-list-boundary-live.spec.ts",
                        "--workers=1",
                    ],
                    cwd=playwright,
                    env={
