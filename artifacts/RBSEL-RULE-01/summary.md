...
Updated `.worktrees/rbsel-rule-01/AGENTS.md` and refreshed `artifacts/RBSEL-RULE-01/**` so they match the live worktree and the scoped baseline-to-HEAD diff.
...
- Confirmed the permission snippet fence was reverted so the `AGENTS.md` change set stays inside the RBSEL-RULE-01 closure slice.
- `./scripts/task_validate.sh RBSEL-RULE-01` passed.
- Task gate output: `Task Gate PASS`.

Scoped diff evidence was regenerated from task baseline `0e78d0` to the current worktree/HEAD contents in `artifacts/RBSEL-RULE-01/git/diff.patch`.
