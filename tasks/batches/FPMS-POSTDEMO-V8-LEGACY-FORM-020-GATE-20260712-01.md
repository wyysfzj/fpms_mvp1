# FPMS Post-Demo V8 Legacy Form-020 Gate
...
- Gate: `DG-LEGACY-FORM-CLASS:form-020`
...
This confirmed negative classification completes the form-020 decision gate without
...
- Task file: `tasks/postdemo/v8/FPMS-V8-LEGACY-FORM-020-MANIFEST-ACTIVATION-20260712-01.md`
- Task file: `tasks/postdemo/v8/FPMS-V8-OUT-020-THIRD-OA-STATEMENT-20260712-01.md`

The activation task is the only current implementation item. The OUT-020 child remains
unstarted until this activation receives current independent acceptance; it then executes as
the only child in the form-020 lane. No other legacy-form task is included.
...
- Only form-020 is owned by this lane: yes
...
behavior, or claim that `三通意见陈述` is a current official form. OUT-020 must apply the accepted
`INTERNAL_ONLY` outcome to form-020 alone, keep it reference-only, and leave every other OUT row
unchanged.
...
product/schema/catalog/ledger scope before OUT-020 may start. This manifest does not self-approve
