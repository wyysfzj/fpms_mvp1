# FPMS Post-Demo V8 Legacy Form-001 Gate
...
- Gate: `DG-LEGACY-FORM-CLASS:form-001`
...
This confirmed negative classification completes the form-001 decision gate without
...
- Task file: `tasks/postdemo/v8/FPMS-V8-LEGACY-FORM-001-MANIFEST-ACTIVATION-20260712-01.md`
- Task file: `tasks/postdemo/v8/FPMS-V8-OUT-001-RECTIFICATION-REPLY-20260712-01.md`

The activation task is the only current implementation item. The OUT-001 child remains
unstarted until this activation receives current independent acceptance; it then executes as
the only child in the form-001 lane. No task from form-002 through form-022 is included.
...
behavior, or claim that `补正答复` is a current official form. OUT-001 must apply the accepted
`INTERNAL_ONLY` outcome to form-001 alone and leave every other OUT row unchanged.
...
product/schema/catalog/ledger scope before OUT-001 may start. This manifest does not self-approve
