# FPMS Post-Demo V8 Legacy Form-003 Gate
...
- Gate: `DG-LEGACY-FORM-CLASS:form-003`
...
This confirmed negative classification completes the form-003 decision gate without
...
- Task file: `tasks/postdemo/v8/FPMS-V8-LEGACY-FORM-003-MANIFEST-ACTIVATION-20260712-01.md`
- Task file: `tasks/postdemo/v8/FPMS-V8-OUT-003-EARLY-PUBLICATION-20260712-01.md`

The activation task is the only current implementation item. The OUT-003 child remains
unstarted until this activation receives current independent acceptance; it then executes as
the only child in the form-003 lane. No other legacy-form task is included.
...
behavior, or claim that `提前公开请求` is a current official form. OUT-003 must apply the accepted
`INTERNAL_ONLY` outcome to form-003 alone and leave every other OUT row unchanged.
...
product/schema/catalog/ledger scope before OUT-003 may start. This manifest does not self-approve
