# FPMS Post-Demo V8 Legacy Form-011 Gate
...
- Gate: `DG-LEGACY-FORM-CLASS:form-011`
...
This confirmed negative classification completes the form-011 decision gate without
...
- Task file: `tasks/postdemo/v8/FPMS-V8-LEGACY-FORM-011-MANIFEST-ACTIVATION-20260712-01.md`
- Task file: `tasks/postdemo/v8/FPMS-V8-OUT-011-REEXAM-INVALIDATION-STATEMENT-20260712-01.md`

The activation task is the only current implementation item. The OUT-011 child remains
unstarted until this activation receives current independent acceptance; it then executes as
the only child in the form-011 lane. No other legacy-form task is included.
...
behavior, or claim that `复审、无效程序中的意见陈述` is a current official form. OUT-011 must
apply the accepted `INTERNAL_ONLY` outcome to form-011 alone and leave every other OUT row
unchanged.
...
product/schema/catalog/ledger scope before OUT-011 may start. This manifest does not self-approve
