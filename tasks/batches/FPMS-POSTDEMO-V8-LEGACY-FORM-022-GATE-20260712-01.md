# FPMS Post-Demo V8 Legacy Form-022 Gate
...
- Gate: `DG-LEGACY-FORM-CLASS:form-022`
...
This confirmed negative classification completes the form-022 decision gate without
...
- Task file: `tasks/postdemo/v8/FPMS-V8-LEGACY-FORM-022-MANIFEST-ACTIVATION-20260712-01.md`
- Task file: `tasks/postdemo/v8/FPMS-V8-OUT-022-FILE-COPY-REQUEST-20260712-01.md`

The activation task is the only current implementation item. The OUT-022 child remains
unstarted until this activation receives current independent acceptance; it then executes as
the only child in the form-022 lane. No task from any other legacy-form lane is included.
...
behavior, or claim that `办理文件副本请求书` is a current official form. OUT-022 must apply the
accepted `INTERNAL_ONLY` outcome to form-022 alone and leave every other OUT row unchanged.
...
product/schema/catalog/ledger scope before OUT-022 may start. This manifest does not self-approve
