# FPMS Post-Demo V8 Legacy Form-006 Gate
...
- Gate: `DG-LEGACY-FORM-CLASS:form-006`
...
This confirmed negative classification completes the form-006 decision gate without
...
- Task file: `tasks/postdemo/v8/FPMS-V8-LEGACY-FORM-006-MANIFEST-ACTIVATION-20260712-01.md`
- Task file: `tasks/postdemo/v8/FPMS-V8-OUT-006-ABANDONMENT-20260712-01.md`

The activation task is the only current implementation item. The OUT-006 child remains
unstarted until this activation receives current independent acceptance; it then executes as
the only child in the form-006 lane. No other legacy-form task is included.
...
behavior, or claim that `主动放弃` is a current official form. OUT-006 must apply the accepted
`INTERNAL_ONLY` outcome to form-006 alone and leave every other OUT row unchanged.
...
product/schema/catalog/ledger scope before OUT-006 may start. This manifest does not self-approve
