# FPMS Post-Demo V8 Legacy Form-018 Gate
...
- Gate: `DG-LEGACY-FORM-CLASS:form-018`
...
This confirmed negative classification completes the form-018 decision gate without
...
- Task file: `tasks/postdemo/v8/FPMS-V8-LEGACY-FORM-018-MANIFEST-ACTIVATION-20260712-01.md`
- Task file: `tasks/postdemo/v8/FPMS-V8-OUT-018-TIME-EXTENSION-20260712-01.md`

The activation task is the only current implementation item. The OUT-018 child remains
unstarted until this activation receives current independent acceptance; it then executes as
the only child in the form-018 lane. No task from any other legacy-form lane is included.
...
behavior, or claim that `延长期限请求` is a current official form. OUT-018 must apply the accepted
`INTERNAL_ONLY` outcome to form-018 alone and leave every other OUT row unchanged.
...
product/schema/catalog/ledger scope before OUT-018 may start. This manifest does not self-approve
