# FPMS Post-Demo V8 Legacy Form-015 Gate
...
- Gate: `DG-LEGACY-FORM-CLASS:form-015`
...
This confirmed negative classification completes the form-015 decision gate without
...
- Task file: `tasks/postdemo/v8/FPMS-V8-LEGACY-FORM-015-MANIFEST-ACTIVATION-20260712-01.md`
- Task file: `tasks/postdemo/v8/FPMS-V8-OUT-015-TRANSLATION-CORRECTION-20260712-01.md`

The activation task is the only current implementation item. The OUT-015 child remains
unstarted until this activation receives current independent acceptance; it then executes as
the only child in the form-015 lane. No other legacy-form task is included.
...
behavior, or claim that `改正译文错误请求书` is a current official form. OUT-015 must apply the
accepted `INTERNAL_ONLY` outcome to form-015 alone and leave every other OUT row unchanged.
...
product/schema/catalog/ledger scope before OUT-015 may start. This manifest does not self-approve
