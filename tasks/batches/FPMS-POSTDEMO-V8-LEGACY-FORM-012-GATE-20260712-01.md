# FPMS Post-Demo V8 Legacy Form-012 Gate
...
- Gate: `DG-LEGACY-FORM-CLASS:form-012`
...
This confirmed negative classification completes the form-012 decision gate without
...
- Task file: `tasks/postdemo/v8/FPMS-V8-LEGACY-FORM-012-MANIFEST-ACTIVATION-20260712-01.md`
- Task file: `tasks/postdemo/v8/FPMS-V8-OUT-012-REEXAMINATION-RECTIFICATION-20260712-01.md`

The activation task is the only current implementation item. The OUT-012 child remains
unstarted until this activation receives current independent acceptance; it then executes as
the only child in the form-012 lane. No other legacy-form task is included.
...
- Only form-012 is owned by this lane: yes
...
behavior, or claim that `复审中的补正` is a current official form. OUT-012 must apply the accepted
`INTERNAL_ONLY` outcome to form-012 alone, keep it reference-only, and leave every other OUT row
unchanged.
...
product/schema/catalog/ledger scope before OUT-012 may start. This manifest does not self-approve
