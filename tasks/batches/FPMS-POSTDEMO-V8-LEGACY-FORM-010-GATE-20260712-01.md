# FPMS Post-Demo V8 Legacy Form-010 Gate
...
- Gate: `DG-LEGACY-FORM-CLASS:form-010`
...
This confirmed negative classification completes the form-010 decision gate without
...
- Task file: `tasks/postdemo/v8/FPMS-V8-LEGACY-FORM-010-MANIFEST-ACTIVATION-20260712-01.md`
- Task file: `tasks/postdemo/v8/FPMS-V8-OUT-010-RIGHT-RESTORATION-20260712-01.md`

The activation task is the only current implementation item. The OUT-010 child remains
unstarted until this activation receives current independent acceptance; it then executes as
the only child in the form-010 lane. No other legacy-form task is included.
...
behavior, or claim that `恢复权利请求` is a current official form. OUT-010 must apply the accepted
`INTERNAL_ONLY` outcome to form-010 alone and leave every other OUT row unchanged.
...
product/schema/catalog/ledger scope before OUT-010 may start. This manifest does not self-approve
