# FPMS Post-Demo V8 Legacy Form-004 Gate
...
- Gate: `DG-LEGACY-FORM-CLASS:form-004`
...
This confirmed negative classification completes the form-004 decision gate without
...
- Task file: `tasks/postdemo/v8/FPMS-V8-LEGACY-FORM-004-MANIFEST-ACTIVATION-20260712-01.md`
- Task file: `tasks/postdemo/v8/FPMS-V8-OUT-004-EXAM-REQUEST-20260712-01.md`

The activation task is the only current implementation item. The OUT-004 child remains
unstarted until this activation receives current independent acceptance; it then executes as
the only child in the form-004 lane. No other legacy-form task is included.
...
behavior, or claim that `实审请求` is a current official form. OUT-004 must apply the accepted
`INTERNAL_ONLY` outcome to form-004 alone and leave every other OUT row unchanged.
...
product/schema/catalog/ledger scope before OUT-004 may start. This manifest does not self-approve
