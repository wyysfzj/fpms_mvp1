# AI‑EOS PROMPT — FE-2-24
## Title
System: Letterheads list/create (for printing chain)
...
## Objective
Implement letterheads maintenance.
- Route: `/system/letterheads`
- List letterheads; allow create.
- This supports bill printing; show clear empty state if none configured.

## API Endpoints (use shared axios client only)
- `GET /letterheads?page=1&page_size=50`
- `POST /letterheads`
...
- Keep it compact; align empty state to overall style.
- Show requestId on errors.

## File Allowlist (ONLY modify/add these)
- `src/api/system.ts (update)`
- `src/api/system.types.ts (update)`
- `src/modules/system/pages/LetterheadList.vue (new)`
- `src/modules/system/components/LetterheadCreate.vue (new)`
- `src/router/index.ts (update)`
- `src/styles/layout.css (optional)`
- Evidence Log:
  - `task/frontend/FE-2/FE-2-24_evidence.md` (add)
...
Write `task/frontend/FE-2/FE-2-24_evidence.md` including:
