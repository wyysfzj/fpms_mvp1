# AI‑EOS PROMPT — FE-2-15
## Title
Fees: Draft Items CRUD (add/edit/delete) with inline totals
...
## Objective
Implement draft items CRUD inside FeeDraftDetail.
- Add item, edit item, delete item.
- Compute and show totals in the UI.
- On API errors (422/409), show clear message + requestId.

## API Endpoints (use shared axios client only)
- `POST /fees/drafts/{id}/items`
- `PUT /fees/items/{item_id}`
...
- Items table should be compact and readable; numbers in mono font if feasible.
- Use dialogs/drawers for edit; avoid clutter.
...
- `src/modules/fees/components/FeeDraftItemsTable.vue (new)`
- `src/styles/layout.css (optional)`
- Evidence Log:
  - `task/frontend/FE-2/FE-2-15_evidence.md` (add)
...
Write `task/frontend/FE-2/FE-2-15_evidence.md` including:
