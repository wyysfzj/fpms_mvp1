# AI‑EOS PROMPT — FE-2-19
## Title
Billing: Bill print/download (blob) + template-missing UX
...
## Objective
Add bill print capability to BillDetail.
- Add a print/download button.
- Call print endpoint, receive blob, open/download.
- If backend returns 409 due to missing template, show a clear configuration-missing message + requestId.

## API Endpoints (use shared axios client only)
- `GET /bills/{id}/print (blob)`
...
- Print button should be placed in the header action area, token-aligned.
- Blob download must not break SPA navigation.

## File Allowlist (ONLY modify/add these)
- `src/api/billing.ts (update)`
- `src/modules/billing/pages/BillDetail.vue (update)`
- `src/styles/layout.css (optional)`
- Evidence Log:
  - `task/frontend/FE-2/FE-2-19_evidence.md` (add)
...
Write `task/frontend/FE-2/FE-2-19_evidence.md` including:
