# AI‑EOS PROMPT — FE-2-11
## Title
Documents: Detail + Edit (supports Focus Mode for long text)
...
## Objective
Implement Document detail view and edit.
- Route: `/documents/:id`
- Show metadata header and body content/notes if available.
- Mark this route as `meta.supportsFocusMode = true` if it contains long-form content.
- Provide edit action (route `/documents/:id/edit` or inline).
...
- Detail layout should follow the reference: meta header + tabs (optional) + content area.
- In Focus Mode, content should center and use `--font-read` with increased line-height.

## File Allowlist (ONLY modify/add these)
- `src/api/documents.ts (update)`
- `src/api/documents.types.ts (update)`
- `src/modules/documents/pages/DocumentDetail.vue (new)`
- `src/modules/documents/pages/DocumentEdit.vue (new) OR reuse a form component`
- `src/router/index.ts (update: add routes + supportsFocusMode meta)`
- `src/styles/layout.css (optional)`
- Evidence Log:
  - `task/frontend/FE-2/FE-2-11_evidence.md` (add)
...
Write `task/frontend/FE-2/FE-2-11_evidence.md` including:
