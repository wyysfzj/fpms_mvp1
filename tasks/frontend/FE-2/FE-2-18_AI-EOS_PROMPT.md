# AI‑EOS PROMPT — FE-2-18
## Title
Billing: Bill create (from drafts + manual)
...
## Objective
Implement bill creation flows.
- Route: `/billing/bills/new`
- Provide two modes:
  - Create from drafts (select draft IDs)
  - Manual create (minimal fields)
- On success, navigate to created bill detail.
...
- Use tabs or segmented control to switch create mode.
- Validate constraints and surface 409 clearly (e.g., currency mismatch).

## File Allowlist (ONLY modify/add these)
- `src/api/billing.ts (update)`
- `src/api/billing.types.ts (update)`
- `src/modules/billing/pages/BillCreate.vue (new)`
- `src/router/index.ts (update)`
- `src/styles/layout.css (optional)`
- Evidence Log:
  - `task/frontend/FE-2/FE-2-18_evidence.md` (add)
...
Write `task/frontend/FE-2/FE-2-18_evidence.md` including:
