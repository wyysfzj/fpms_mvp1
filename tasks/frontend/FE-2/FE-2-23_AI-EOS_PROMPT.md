# AI‑EOS PROMPT — FE-2-23
## Title
System: Params list + upsert (key/value) with 422/409 UX
...
## Objective
Implement system params.
- Route: `/system/params`
- List params and allow upsert (edit/save per key).
- Map 422 to field errors; 409 to conflict banner.
...
- Use table with inline edit row; keep UI minimal and auditable.
- Keys/IDs in mono font if feasible.

## File Allowlist (ONLY modify/add these)
- `src/api/system.ts (update)`
- `src/api/system.types.ts (update)`
- `src/modules/system/pages/SystemParams.vue (new)`
- `src/router/index.ts (update)`
- `src/styles/layout.css (optional)`
- Evidence Log:
  - `task/frontend/FE-2/FE-2-23_evidence.md` (add)
...
Write `task/frontend/FE-2/FE-2-23_evidence.md` including:
