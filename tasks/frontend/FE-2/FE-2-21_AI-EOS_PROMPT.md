# AI‑EOS PROMPT — FE-2-21
## Title
Billing: Case receipts summary (CaseDetail Billing tab integration)
...
## Objective
Integrate receipts summary into Case detail.
- Add a Billing tab section to CaseDetail that calls receipts endpoint and shows summary.
- Keep it compact and token-aligned.
- If the CaseDetail tabs are scaffolded, wire this tab to real data.
...
- Receipts section should align with reference side-panel/section styling.
- In Focus Mode, this section may be hidden or de-emphasized to maintain reading flow.

## File Allowlist (ONLY modify/add these)
- `src/api/billing.ts (update) OR src/api/cases.ts (update: add receipts call)`
- `src/modules/cases/pages/CaseDetail.vue (update)`
- `src/modules/cases/components/CaseReceiptsSummary.vue (new)`
- `src/styles/layout.css (optional)`
- Evidence Log:
  - `task/frontend/FE-2/FE-2-21_evidence.md` (add)
...
Write `task/frontend/FE-2/FE-2-21_evidence.md` including:
