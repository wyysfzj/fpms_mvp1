# SKELE2E-HARNESS-BIODEPOSIT-01 — Skeleton A002 bio deposit table DB assertion

Task ID: `SKELE2E-HARNESS-BIODEPOSIT-01`

## Exact Closure Slice

Update only the FPMS Automation Skeleton Pack `TC-A-002` DB assertion contract so it checks the product bio-deposit table `t_bio_deposit` instead of the non-existent legacy table name `t_case_bio_deposit`.

This task closes only:

1. `TC-A-002` uses the current product table name for bio-deposit rows.
2. The focused handler unit test expects the same table name.
3. The API payload, detail assertions, priority assertions, and product backend behavior remain unchanged.

## Explicit Non-Closure

No product backend changes. No database schema or migration changes. No frontend changes.
Do not change case creation payloads, bio-deposit validation, priority assertions, or any non-`TC-A-002` handler behavior.
...
- `tasks/postenhancement/e2e/SKELE2E-HARNESS-BIODEPOSIT-01.md`
...
- `artifacts/SKELE2E-HARNESS-BIODEPOSIT-01/**`

## Verification Commands

- `python3 /Users/cfcc/.codex/skills/atomic-evidence-gates/scripts/evidence_gate.py check-task tasks/postenhancement/e2e/SKELE2E-HARNESS-BIODEPOSIT-01.md`
...
- `python3 /Users/cfcc/.codex/skills/atomic-evidence-gates/scripts/evidence_gate.py validate SKELE2E-HARNESS-BIODEPOSIT-01`
- `./scripts/task_validate.sh SKELE2E-HARNESS-BIODEPOSIT-01`
...
- Focused tests prove `TC-A-002` expects `t_bio_deposit` for bio-deposit DB rows.
- `TC-A-002` handler asserts `t_bio_deposit` in real DB mode.
