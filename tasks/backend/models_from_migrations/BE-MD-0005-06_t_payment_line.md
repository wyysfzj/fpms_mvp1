# BE-MD-0005-06_t_payment_line — ORM model for table `t_payment_line`
    ...
    - **Atomic rule:** modify/create ONLY this file; implement ONLY ONE ORM class for `t_payment_line`.

    ## Scope decision (MVP1 – FIXED)
    - Scope is FIXED for MVP1.
    - Implement ONLY the ORM class mapping to `t_payment_line`.
    - `__tablename__` MUST be exactly `t_payment_line` (lowercase, matches migration).
    ...
    ### Table: `t_payment_line`
    Recommended class name: `PaymentLine` (name can vary, table mapping must match)

    **Columns (EXACT, from migration):**
    - `id` : `sa.String(36`  (→ `str`)
- `payment_id` : `sa.String(36`  (→ `str`)
- `case_id` : `sa.String(36`  (→ `str`)
- `raw_amount` : `sa.Numeric(18`  (→ `Decimal`)
- `allocated_amt` : `sa.Numeric(18`  (→ `Decimal`)
- `balance_amt` : `sa.Numeric(18`  (→ `Decimal`)
...
- `t_case.id`
    ...
    **Indexes (defined in migrations):**
    ...
    ## Prompt
    In `backend/app/modules/billing/models.py`, implement ONE SQLAlchemy model class that maps EXACTLY to table `t_payment_line` as defined in `backend/alembic/versions/0005_billing.py`.
