# BE-MD-0001-03_t_role — ORM model for table `t_role`
    ...
    - **Atomic rule:** modify/create ONLY this file; implement ONLY ONE ORM class for `t_role`.

    ## Scope decision (MVP1 – FIXED)
    - Scope is FIXED for MVP1.
    - Implement ONLY the ORM class mapping to `t_role`.
    - `__tablename__` MUST be exactly `t_role` (lowercase, matches migration).
    ...
    ### Table: `t_role`
    Recommended class name: `Role` (name can vary, table mapping must match)

    **Columns (EXACT, from migration):**
    - `id` : `sa.String(36`  (→ `str`)
- `code` : `sa.String(64`  (→ `str`)
- `name` : `sa.String(128`  (→ `str`)
    ...
    **Indexes (defined in migrations):**
    ...
    ## Prompt
    In `backend/app/modules/auth/models.py`, implement ONE SQLAlchemy model class that maps EXACTLY to table `t_role` as defined in `backend/alembic/versions/0001_mvp1_core_tables.py`.
