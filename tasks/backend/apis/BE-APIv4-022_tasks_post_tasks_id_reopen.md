# BE-APIv4-022_tasks_post_tasks_id_reopen — tasks POST /tasks/{id}/reopen
...
- Implement ONLY `POST /tasks/{id}/reopen`.
...
curl -i -H "Authorization: Bearer $FPMS_TOKEN" -H "Content-Type: application/json" -d '{}' "http://localhost:8000/api/v1/tasks/REPLACE_ID/reopen"
```

## Prompt
In `backend/app/modules/tasks/api.py`, implement ONLY the endpoint `POST /tasks/{id}/reopen` according to `tasks/docs/task_02_api.md`.
