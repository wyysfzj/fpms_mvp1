# BE-APIv4-038_billing_post_bills_from-drafts — billing POST /bills/from-drafts
...
curl -i -H "Authorization: Bearer $FPMS_TOKEN" -H "Content-Type: application/json" -d '{}' "http://localhost:8000/api/v1/bills/from-drafts"
```

## Prompt
In `backend/app/modules/billing/api.py`, implement ONLY the endpoint `POST /bills/from-drafts` according to `billing/docs/bill_02_api.md`.
