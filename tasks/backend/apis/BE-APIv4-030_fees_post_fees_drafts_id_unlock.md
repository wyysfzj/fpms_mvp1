# BE-APIv4-030_fees_post_fees_drafts_id_unlock — fees POST /fees/drafts/{id}/unlock
...
curl -i -H "Authorization: Bearer $FPMS_TOKEN" -H "Content-Type: application/json" -d '{}' "http://localhost:8000/api/v1/fees/drafts/REPLACE_ID/unlock"
```

## Prompt
In `backend/app/modules/fees/api.py`, implement ONLY the endpoint `POST /fees/drafts/{id}/unlock` according to `fees/docs/fee_02_api.md`.
