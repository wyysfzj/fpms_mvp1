...
- Implement ONLY `GET /tasks/today?as=worker|supervisor`.
- No schema changes, no new tables, no migration changes.
- Use existing ORM models only.

## Endpoint (EXACT)
- Method: `GET`
- Path: `/tasks/today?as=worker|supervisor`
...
curl -i -H "Authorization: Bearer $FPMS_TOKEN" "http://localhost:8000/api/v1/tasks/today?as=worker|supervisor"
```

## Prompt
In `backend/app/modules/tasks/api.py`, implement ONLY the endpoint `GET /tasks/today?as=worker|supervisor` according to `tasks/docs/task_02_api.md`.
