...
Activate a version only with CNIPA source snapshot, approval, effective interval and non-overlap; customer sheets never activate it.
...
- `backend/app/modules/fees/official_rate_book.py` order key `1`; project this order only across owners present in the active manifest.
