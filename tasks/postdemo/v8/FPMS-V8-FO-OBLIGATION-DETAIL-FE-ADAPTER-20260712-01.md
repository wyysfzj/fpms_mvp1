...
Type/fetch one obligation detail, preserving decimal strings and separated states.
...
- Approved source dependency cell (verbatim): detail HTTP; serialized after instruction FE adapter

### Shared ownership serialization

- `frontend/src/api/fees.ts` order key `4`; project this order only across owners present in the active manifest.
- `frontend/src/api/fees.types.ts` order key `4`; project this order only across owners present in the active manifest.
- `FRONTEND_TYPECHECK_VERIFICATION` order key `4`; project this order only across owners present in the active manifest.
...
- Evidence validation: `python3 /Users/cfcc/.codex/skills/atomic-evidence-gates/scripts/evidence_gate.py validate FPMS-V8-FO-OBLIGATION-DETAIL-FE-ADAPTER-20260712-01 --required-step lint --required-step test --required-step independent_review --required-step scope`
