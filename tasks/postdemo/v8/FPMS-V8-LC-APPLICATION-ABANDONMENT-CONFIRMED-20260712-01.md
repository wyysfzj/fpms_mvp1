...
Un-granted application enters abandoned and closed.
...
- `backend/app/modules/cases/lifecycle_rules.py` order key `19`; project this order only across owners present in the active manifest.
...
- RED command: `cd backend && .venv/bin/pytest -q tests/test_v8_lifecycle_application_abandonment.py`; run it before implementation and preserve the expected failure proving the named missing behavior.
...
- Evidence validation: `python3 /Users/cfcc/.codex/skills/atomic-evidence-gates/scripts/evidence_gate.py validate FPMS-V8-LC-APPLICATION-ABANDONMENT-CONFIRMED-20260712-01 --required-step lint --required-step test --required-step independent_review --required-step scope`
