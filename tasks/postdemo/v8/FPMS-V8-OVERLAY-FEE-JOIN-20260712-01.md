...
Bulk attach obligation/instruction/draft/list/payment/evidence facts by activity IDs.
...
- Approved source dependency cell (verbatim): document join; fee core

### Shared ownership serialization

- `backend/app/modules/cases/lifecycle_overlay_service.py` order key `3`; project this order only across owners present in the active manifest.
...
- RED command: `cd backend && .venv/bin/pytest -q tests/test_v8_lifecycle_overlay_fees.py`; run it before implementation and preserve the expected failure proving the named missing behavior.
...
- Evidence validation: `python3 /Users/cfcc/.codex/skills/atomic-evidence-gates/scripts/evidence_gate.py validate FPMS-V8-OVERLAY-FEE-JOIN-20260712-01 --required-step lint --required-step test --required-step independent_review --required-step scope`
