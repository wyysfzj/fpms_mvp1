...
from handlers.wave_x import handle_tc_x_013, handle_tc_x_014
...
def test_tc_x_014_handler_creates_manual_ap_bill_with_two_items() -> None:
    runtime = FakeRuntime(
        username="admin",
    ...
    handle_tc_x_014(runtime, _case())  # type: ignore[arg-type]

    post_paths = [
        call["path"] for call in runtime.api.calls if call["method"] == "POST"
    ]
    assert post_paths == ["/clients", "/bills/manual"]
    payload = runtime.api.calls[-2]["kwargs"]["json"]
    assert payload["direction"] == "AP"
    assert len(payload["items"]) == 2
    bill = runtime.api.bills[0]
    assert bill["direction"] == "AP"
    assert bill["amount"] == "500.00"


def test_tc_x_014_handler_runs_db_assert_when_enabled() -> None:
    runtime = FakeRuntime(
        username="admin",
        password="dummy-password",
        run_id="RUN-X-AP-DB",
    ...
    assert runtime.db.rows == [("t_bill", {"id": "bill-1", "direction": "AP"})]


def test_tc_x_014_is_registered_as_real_handler() -> None:
    assert not getattr(handle_tc_x_014, "_is_skeleton", False)
