...
test('费用节点线无损展示 GOV 和 SERVICE 义务的七个独立状态', async ({ page }) => {
    ...
    await expect(lane.getByRole('heading', { name: '费用节点' })).toBeVisible()
    const gov = lane.getByTestId('fee-obligation-obligation-gov')
    await expect(gov.getByText('费用域：GOV', { exact: true })).toBeVisible()
    await expect(gov.getByText('估算状态：ESTIMATE', { exact: true })).toBeVisible()
    await expect(gov.getByText('义务状态：RECOGNIZED', { exact: true })).toBeVisible()
    await expect(gov.getByText('客户指示状态：PAY', { exact: true })).toBeVisible()
    await expect(gov.getByText('草单状态：CREATED', { exact: true })).toBeVisible()
    await expect(gov.getByText('缴费清单状态：CREATED', { exact: true })).toBeVisible()
    await expect(gov.getByText('付款状态：PAID', { exact: true })).toBeVisible()
    await expect(gov.getByText('官方证据状态：VERIFIED', { exact: true })).toBeVisible()
    ...
    await expect(gov.getByText('关联事实：PAYMENT / payment-1 / PAID', { exact: true })).toBeVisible()
    await expect(lane.getByTestId('fee-obligation-obligation-service')).toContainText('费用域：SERVICE')
